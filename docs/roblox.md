...
The Roblox helper currently exposes grade and resistance commands. The core HTTP protocol and browser bridge support every target-setting procedure advertised by a connected FTMS indoor bike. The browser bridge must be connected, the user must enable `Enable trainer control`, and the matching permission toggle must remain enabled. Treadmill speed and incline commands are intentionally not exposed by the Roblox helper and are blocked by the web bridge if sent manually.
