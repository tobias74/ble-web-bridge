...
test('parses cycling power and derives cadence from crank revolution deltas', () => {
  ...
  const repeatedEvent = parseCyclingPowerMeasurement(view([
    0x20, 0x00,
    0x04, 0x01,
    0xea, 0x03,
    0xe8, 0x07
  ]), state);
  assert.equal(Object.hasOwn(repeatedEvent.values, 'cadenceRpm'), false);
});
...
test('exposes FTMS and plugin control protocols as distinct targets', () => {
  const plugin = {
    manifest: {
      id: 'vendor.control',
      label: 'Vendor control',
      protocols: [{ id: 'vendor.protocol', label: 'Vendor Protocol' }],
      handledCommandTypes: ['bike.resistance'],
      commands: []
    }
  };
  ...
  const targets = createDeviceControlTargets({
    deviceId: 'dev_1',
    deviceKey: 'trainer-001',
    deviceName: 'Test Trainer'
  }, {
    controlPoint: {},
    isIndoorBike: true,
    targetSettingFeatures: 1 << 2
  }, pluginConnections);

  assert.deepEqual(targets.map(({ id, protocol, protocolLabel }) => ({ id, protocol, protocolLabel })), [
    {
      id: 'trainer-001::ftms',
      protocol: 'ftms',
    ...
    {
      id: 'trainer-001::vendor.protocol',
      protocol: 'vendor.protocol',
      protocolLabel: 'Vendor Protocol'
    }
  ]);
});
...
test('parses cycling speed/cadence crank data', () => {
  ...
  const repeatedEvent = parseCyclingSpeedCadenceMeasurement(view([
    0x02,
    0x0c, 0x00,
    0xe8, 0x07
  ]), state);
...
test('parses battery and device information metadata', () => {
  ...
  assert.equal(parseDeviceInfoString(textView('EXAMPLE TRAINER\0')), 'EXAMPLE TRAINER');
});

test('reads every FTMS target setting feature advertised by a trainer', () => {
  const features = parseFitnessMachineFeatures(view([
    0x00, 0x00, 0x00, 0x00,
    0xff, 0xff, 0x01, 0x00
  ]));
  assert.equal(features.targetSettingFeatures, 0x1ffff);

  const capabilities = getControlCapabilities({
    isIndoorBike: true,
    isTreadmill: false,
    controlPoint: {},
    targetSettingFeatures: features.targetSettingFeatures
  });
  const capabilityKeys = [
    ...
    'canWriteResistance',
    'canWriteTargetPower',
    'canWriteTargetHeartRate',
    'canWriteTargetEnergy',
    'canWriteTargetSteps',
    'canWriteTargetStrides',
    'canWriteTargetDistance',
    'canWriteTargetTrainingTime',
    'canWriteTargetTimeTwoHeartRateZones',
    'canWriteTargetTimeThreeHeartRateZones',
    'canWriteTargetTimeFiveHeartRateZones',
    'canWriteGrade',
    'canWriteWheelCircumference',
    'canWriteSpinDown',
    'canWriteTargetCadence'
  ];
  capabilityKeys.forEach((key) => assert.equal(capabilities[key], true, key));
});

test('keeps basic trainer control visible for legacy FTMS devices without target feature bits', () => {
  const capabilities = getControlCapabilities({
    isIndoorBike: true,
    isTreadmill: false,
    controlPoint: {},
  ...
  assert.equal(capabilities.canWriteGrade, true);
  assert.equal(capabilities.canWriteResistance, true);
  assert.equal(capabilities.canWriteTargetPower, false);
  assert.equal(capabilities.canWriteBike, true);
});

test('serializes all FTMS target setting procedures', () => {
  const cases = [
    [{ type: 'bike.targetSpeed', targetSpeedMps: 10 }, [0x02, 0x10, 0x0e]],
    [{ type: 'bike.inclination', inclinePct: 5.5 }, [0x03, 0x37, 0x00]],
    [{ type: 'bike.resistance', resistanceLevel: 35 }, [0x04, 0x5e, 0x01]],
    [{ type: 'bike.targetPower', targetPowerW: 250 }, [0x05, 0xfa, 0x00]],
    [{ type: 'bike.targetHeartRate', targetHeartBpm: 150 }, [0x06, 0x96]],
    [{ type: 'bike.targetEnergy', targetEnergyKcal: 500 }, [0x09, 0xf4, 0x01]],
    [{ type: 'bike.targetSteps', targetSteps: 2000 }, [0x0a, 0xd0, 0x07]],
    [{ type: 'bike.targetStrides', targetStrides: 1234 }, [0x0b, 0xd2, 0x04]],
    [{ type: 'bike.targetDistance', targetDistanceM: 5000 }, [0x0c, 0x88, 0x13, 0x00]],
    [{ type: 'bike.targetTrainingTime', targetTrainingTimeS: 3600 }, [0x0d, 0x10, 0x0e]],
    [{ type: 'bike.targetTimeTwoHeartRateZones', zoneTimesS: [1800, 1800] }, [0x0e, 0x08, 0x07, 0x08, 0x07]],
    [{ type: 'bike.targetTimeThreeHeartRateZones', zoneTimesS: [500, 600, 1800] }, [0x0f, 0xf4, 0x01, 0x58, 0x02, 0x08, 0x07]],
    [{ type: 'bike.targetTimeFiveHeartRateZones', zoneTimesS: [600, 1200, 1200, 600, 300] }, [0x10, 0x58, 0x02, 0xb0, 0x04, 0xb0, 0x04, 0x58, 0x02, 0x2c, 0x01]],
    [{ type: 'bike.grade', gradePct: 4.5 }, [0x11, 0x00, 0x00, 0xc2, 0x01, 0x28, 0x33]],
    [{ type: 'bike.wheelCircumference', wheelCircumferenceMm: 2100 }, [0x12, 0x08, 0x52]],
    [{ type: 'bike.spinDown', spinDownAction: 'start' }, [0x13, 0x01]],
    [{ type: 'bike.targetCadence', targetCadenceRpm: 90 }, [0x14, 0xb4, 0x00]]
  ];

  for (const [command, expected] of cases) {
    assert.deepEqual([...serializeFtmsCommand(command).bytes], expected, command.type);
  }
});
